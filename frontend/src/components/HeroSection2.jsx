import React from "react";
import HeadAndDetails from "./HeadAndDetails";
import CubeCards from "./CubeCards";
import { Link } from "react-router";
const HeroSection2 = () => {
  const svg1 = (
    <svg
      width="64px"
      height="64px"
      viewBox="0 0 1024 1024"
      className="icon"
      xmlns="http://www.w3.org/2000/svg"
      fill="#000000"
      // {...props}
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <path
          d="M554.445166 196.193834h-83.958596v83.958597h83.958596v-83.958597z m-26.621018 55.289808h-28.668789v-28.668789h28.668789v28.668789zM596.424464 727.590318v13.310509h-167.917193v-13.310509h-26.621018v23.00056c0 11.570928 11.861711 18.978738 23.432639 18.978738h181.863536c11.570928 0 17.909802-7.406786 17.909802-18.978738v-23.00056h-28.667766z"
          fill="#62C9B6"
        ></path>
        <path
          d="M485.844849 461.380133h-225.254771V337.900587c0-7.694498 10.144656-15.768858 17.839154-15.768858h195.85288c7.694498 0 11.561713 8.07436 11.561713 15.768858v123.479546zM764.341658 461.380133h-223.207001V337.900587c0-7.694498 9.391076-15.768858 17.085574-15.768858h195.852881c7.694498 0 10.268546 8.07436 10.268546 15.768858v123.479546z"
          fill="#62C9B6"
        ></path>
        <path
          d="M609.734974 727.590318v23.00056c0 3.846737 1.295215 3.620459-2.551523 3.620458H425.319916c-3.846737 0-10.12213 0.227303-10.12213-3.620458v-23.00056"
          fill="#62C9B6"
        ></path>
        <path
          d="M78.33849 754.211336h112.627386v28.668789h-112.627386zM834.989745 754.211336h111.603501v28.668789h-111.603501zM891.303438 727.590318h-28.668789V267.952837c0-23.14288-15.470907-45.137985-38.613787-45.137984h-615.53938c-23.14288 0-46.184395 21.995105-46.184395 45.137984v459.637481h-26.621019V267.952837c0-38.56976 34.238725-71.759003 72.805414-71.759003h615.53938c38.566689 0 67.282576 33.188219 67.282576 71.759003v459.637481z"
          fill="#62C9B6"
        ></path>
        <path
          d="M907.957957 824.859424H124.544387c-30.854784 0-59.516406-22.429232-59.516406-53.284016v-27.978691c0-15.430976 16.10674-31.364679 31.537716-31.364679h839.371974c15.430976 0 23.967108 15.933703 23.967108 31.364679v27.978691c-0.001024 30.855808-21.092038 53.284016-51.946822 53.284016z m-814.261187-83.958597v30.674581c0 15.430976 15.416641 24.615227 30.847617 24.615227h783.41357c15.430976 0 25.32478-9.184251 25.324779-24.615227v-30.674581h-839.585966zM834.989745 685.61102h-644.023869V295.931528c0-23.14288 22.352441-44.447886 45.49532-44.447886h559.580975c23.14288 0 38.947574 21.305006 38.947574 44.447886v389.679492z m-615.35508-28.66879h586.686291V295.931528c0-7.714976-2.566881-15.779097-10.278785-15.779097H236.461196c-7.711904 0-16.826531 8.064121-16.826531 15.779097v361.010702z"
          fill="#62C9B6"
        ></path>
        <path
          d="M260.590078 489.025037h225.254771v27.644904h-225.254771zM302.569376 545.33873h141.296175v27.644904h-141.296175zM541.134657 489.025037h223.207001v27.644904h-223.207001zM583.113955 545.33873h139.248404v27.644904h-139.248404z"
          fill="#62C9B6"
        ></path>
      </g>
    </svg>
  );

  const svg2 = (
    <svg
      width="50px"
      height="50px"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <path
          opacity="0.5"
          d="M22 19H14M2 19H10"
          stroke="#62C9B6"
          strokeWidth="1.5"
          strokeLinecap="round"
        ></path>
        <path
          opacity="0.5"
          d="M12 17V14"
          stroke="#62C9B6"
          strokeWidth="1.5"
          strokeLinecap="round"
        ></path>
        <circle
          cx="12"
          cy="19"
          r="2"
          stroke="#62C9B6"
          strokeWidth="1.5"
        ></circle>
        <path
          d="M2 11C2 9.34315 3.34315 8 5 8H19C20.6569 8 22 9.34315 22 11C22 12.6569 20.6569 14 19 14H5C3.34315 14 2 12.6569 2 11Z"
          stroke="#62C9B6"
          strokeWidth="1.5"
        ></path>
        <path
          d="M2 5C2 3.34315 3.34315 2 5 2H19C20.6569 2 22 3.34315 22 5C22 6.65685 20.6569 8 19 8H5C3.34315 8 2 6.65685 2 5Z"
          stroke="#62C9B6"
          strokeWidth="1.5"
        ></path>
        <path
          opacity="0.5"
          d="M13 5L19 5"
          stroke="#62C9B6"
          strokeWidth="1.5"
          strokeLinecap="round"
        ></path>
        <path
          opacity="0.5"
          d="M13 11L19 11"
          stroke="#62C9B6"
          strokeWidth="1.5"
          strokeLinecap="round"
        ></path>
        <circle opacity="0.5" cx="6" cy="5" r="1" fill="#62C9B6"></circle>
        <circle opacity="0.5" cx="6" cy="11" r="1" fill="#62C9B6"></circle>
      </g>
    </svg>
  );

  const svg3 = (
    <svg
      height="45px"
      width="45px"
      version="1.1"
      id="_x32_"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      viewBox="0 0 512 512"
      xmlSpace="preserve"
      fill="#000000"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <style type="text/css">{`.st0{fill:#62C9B6;}`}</style>
        <g>
          <path
            className="st0"
            d="M459.991,82.262c-5.818-13.388-15.866-24.916-28.61-34.854c-19.179-14.896-44.758-26.522-74.674-34.691 C326.789,4.575,292.498,0,256,0c-55.622,0.036-106.071,10.548-143.848,28.374c-18.889,8.959-34.692,19.76-46.31,32.713 c-5.791,6.48-10.52,13.542-13.832,21.176c-3.304,7.634-5.146,15.866-5.138,24.29V405.44c-0.008,8.423,1.834,16.665,5.138,24.29 c5.818,13.388,15.866,24.915,28.61,34.863c19.18,14.896,44.757,26.523,74.675,34.692C185.211,507.425,219.503,511.992,256,512 c55.632-0.046,106.07-10.546,143.848-28.383c18.888-8.958,34.691-19.751,46.309-32.712c5.792-6.472,10.52-13.534,13.833-21.176 c3.304-7.624,5.146-15.867,5.137-24.29V106.552C465.137,98.128,463.295,89.896,459.991,82.262z M436.282,405.44 c-0.009,4.293-0.889,8.478-2.768,12.843c-3.268,7.597-9.849,15.766-19.861,23.545c-14.967,11.7-37.369,22.257-64.526,29.626 c-27.158,7.388-59.054,11.709-93.128,11.7c-51.909,0.046-98.818-10.084-131.503-25.606c-16.357-7.715-29.064-16.774-37.142-25.86 c-4.066-4.538-6.989-9.058-8.868-13.406c-1.88-4.366-2.76-8.55-2.769-12.843v-49.36c16.684,15.25,40.111,27.43,68.13,36.435 c32.304,10.32,70.789,16.247,112.152,16.256c55.16-0.027,105.164-10.493,142.306-28.029c14.931-7.08,27.693-15.368,37.976-24.798 V405.44z M436.282,321.198c-2.051,4.402-4.983,8.814-8.94,13.27c-12.78,14.396-35.917,27.948-65.77,37.442 c-29.844,9.548-66.287,15.23-105.572,15.222c-52.372,0.027-99.726-10.14-133.046-25.95c-16.674-7.87-29.763-17.137-38.294-26.714 c-3.958-4.456-6.89-8.868-8.942-13.26v-64.754c16.684,15.249,40.111,27.43,68.13,36.434 c32.304,10.311,70.789,16.238,112.152,16.248c55.16-0.028,105.164-10.492,142.306-28.03c14.931-7.08,27.693-15.357,37.976-24.788 V321.198z M436.282,221.572c-2.051,4.402-4.983,8.813-8.94,13.261c-12.78,14.395-35.917,27.956-65.77,37.441 c-29.844,9.549-66.287,15.231-105.572,15.222c-52.372,0.028-99.726-10.138-133.046-25.941 c-16.674-7.87-29.763-17.146-38.294-26.722c-3.958-4.448-6.89-8.859-8.942-13.261v-64.754c16.684,15.258,40.111,27.43,68.13,36.443 c32.304,10.32,70.789,16.247,112.152,16.247c55.16-0.027,105.164-10.484,142.306-28.029c14.931-7.08,27.693-15.358,37.976-24.789 V221.572z M436.282,121.936c-2.051,4.403-4.983,8.823-8.94,13.271c-12.78,14.405-35.917,27.956-65.77,37.451 c-29.844,9.548-66.287,15.23-105.572,15.222c-52.372,0.027-99.726-10.14-133.046-25.95c-16.674-7.879-29.763-17.147-38.294-26.722 c-3.958-4.448-6.89-8.868-8.942-13.262v-15.394c0.01-4.294,0.89-8.469,2.769-12.835c3.268-7.607,9.848-15.766,19.86-23.554 c14.968-11.7,37.369-22.256,64.526-29.617c27.158-7.398,59.053-11.709,93.127-11.7c51.91-0.046,98.819,10.085,131.504,25.606 c16.356,7.706,29.054,16.765,37.142,25.86c4.066,4.538,6.988,9.05,8.868,13.406c1.878,4.366,2.758,8.541,2.768,12.835V121.936z"
          ></path>
        </g>
      </g>
    </svg>
  );

  const svg4 = (
    <svg
      width="60px"
      height="60px"
      viewBox="0 0 60 60"
      xmlns="http://www.w3.org/2000/svg"
      fill="#000000"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <defs>
          <style>{".cls-1{fill:none;}.cls-2{fill:#62C9B6;}"}</style>
        </defs>
        <title></title>
        <g data-name="Layer 2" id="Layer_2">
          <g id="Icons">
            <g id="Development">
              <rect className="cls-1" height="60" width="60"></rect>
              <circle className="cls-2" cx="9.33" cy="15.74" r="1"></circle>
              <circle className="cls-2" cx="16" cy="15.74" r="1"></circle>
              <circle className="cls-2" cx="12.67" cy="15.74" r="1"></circle>
              <path
                className="cls-2"
                d="M20,30.25l3-3a1,1,0,1,0-1.42-1.42l-3.72,3.73a1,1,0,0,0,0,1.41l3.72,3.73a1,1,0,0,0,1.42,0,1,1,0,0,0,0-1.41Z"
              ></path>
              <path
                className="cls-2"
                d="M34.32,34.68a1,1,0,0,0,1.41,0L39.46,31a1,1,0,0,0,0-1.41l-3.73-3.73a1,1,0,0,0-1.41,1.42l3,3-3,3A1,1,0,0,0,34.32,34.68Z"
              ></path>
              <path
                className="cls-2"
                d="M31,26.15a1,1,0,0,0-1.37.37L25.94,33a1,1,0,1,0,1.73,1l3.73-6.45A1,1,0,0,0,31,26.15Z"
              ></path>
              <path
                className="cls-2"
                d="M55.86,40a.29.29,0,0,1-.27-.19,10.43,10.43,0,0,0-1.67-2.89.29.29,0,0,1,0-.33,2.3,2.3,0,0,0-.84-3.14L52.33,33V14.74a3,3,0,0,0-3-3H8a3,3,0,0,0-3,3V40.08a2.34,2.34,0,0,0,2.33,2.33H21l-1,3.7H18.67a4.33,4.33,0,0,0-4.21,3.33H12a1,1,0,0,0,0,2H37.47a2.12,2.12,0,0,0,.07.71,2.25,2.25,0,0,0,1.07,1.4l2.11,1.22a2.3,2.3,0,0,0,3.14-.85.31.31,0,0,1,.31-.14,9.74,9.74,0,0,0,3.32,0,.3.3,0,0,1,.31.15,2.3,2.3,0,0,0,2,1.14,2.27,2.27,0,0,0,1.15-.3l2.11-1.22a2.29,2.29,0,0,0,1.07-1.4,2.26,2.26,0,0,0-.23-1.74.3.3,0,0,1,0-.34,10.52,10.52,0,0,0,1.67-2.88.28.28,0,0,1,.27-.19,2.31,2.31,0,0,0,2.3-2.3V42.26A2.3,2.3,0,0,0,55.86,40ZM7,14.74a1,1,0,0,1,1-1H49.33a1,1,0,0,1,1,1v3H7Zm.33,25.67A.33.33,0,0,1,7,40.08V19.74H50.33V32A2.29,2.29,0,0,0,47.8,33h0a.29.29,0,0,1-.3.13,10.27,10.27,0,0,0-3.32,0,.28.28,0,0,1-.3-.13h0a2.29,2.29,0,0,0-3.14-.84l-2.11,1.22a2.24,2.24,0,0,0-1.07,1.39,2.32,2.32,0,0,0,.22,1.75.27.27,0,0,1,0,.33,10.43,10.43,0,0,0-1.67,2.89.29.29,0,0,1-.27.19,2.29,2.29,0,0,0-1.36.45ZM34,46.11H22l1-3.7H33.5V44.7A2.32,2.32,0,0,0,34,46.11Zm-15.32,2H36.48a11,11,0,0,0,.78,1.33H16.57A2.33,2.33,0,0,1,18.67,48.11ZM56.16,44.7a.3.3,0,0,1-.3.3,2.31,2.31,0,0,0-2.15,1.48,8.46,8.46,0,0,1-1.34,2.33,2.31,2.31,0,0,0-.21,2.6.26.26,0,0,1,0,.22.29.29,0,0,1-.14.19L49.94,53a.3.3,0,0,1-.41-.12,2.31,2.31,0,0,0-2.37-1.11,7.92,7.92,0,0,1-2.66,0,2.31,2.31,0,0,0-2.37,1.12.31.31,0,0,1-.41.11l-2.11-1.22a.29.29,0,0,1-.14-.19.26.26,0,0,1,0-.22,2.33,2.33,0,0,0-.21-2.6,8.35,8.35,0,0,1-1.35-2.33A2.28,2.28,0,0,0,35.8,45a.3.3,0,0,1-.3-.3V42.26a.29.29,0,0,1,.3-.3,2.3,2.3,0,0,0,2.14-1.48,8.21,8.21,0,0,1,1.35-2.33,2.33,2.33,0,0,0,.21-2.6.28.28,0,0,1,0-.23.26.26,0,0,1,.14-.18l2.11-1.22a.28.28,0,0,1,.15,0,.3.3,0,0,1,.25.14h0a2.32,2.32,0,0,0,2.37,1.12,7.92,7.92,0,0,1,2.66,0A2.33,2.33,0,0,0,49.53,34h0a.29.29,0,0,1,.4-.1l2.11,1.22a.26.26,0,0,1,.14.18.28.28,0,0,1,0,.23,2.3,2.3,0,0,0,.21,2.59,8.51,8.51,0,0,1,1.34,2.34A2.32,2.32,0,0,0,55.86,42a.29.29,0,0,1,.3.3Z"
              ></path>
              <path
                className="cls-2"
                d="M45.83,38.94a4.54,4.54,0,1,0,4.54,4.54A4.54,4.54,0,0,0,45.83,38.94Zm0,7.08a2.54,2.54,0,1,1,2.54-2.54A2.54,2.54,0,0,1,45.83,46Z"
              ></path>
            </g>
          </g>
        </g>
      </g>
    </svg>
  );
  return (
    <>
      <div className="hero-section2 flex-row">
        <div className="head-details-and-btn">
          <HeadAndDetails
            shortHead={"My Services"}
            mainHead={"WHAT I "}
            spantext={"DO"}
            mainHead2={"?"}
            para={
              "I specialize in building modern, responsive, and high-performance web applications using the MERN stack — MongoDB, Express.js, React.js, and Node.js. From crafting seamless user interfaces to developing secure backend APIs and integrating dynamic databases, i offer complete end-to-end web development solutions tailored to your needs."
            }
          />
          <button className="hire-me">
            <Link to={"/Resume/UZAIR.pdf"} target="_blank">My Resume</Link>
          </button>
        </div>
        <div className="hero2-cube-container">
          <div className="cube-inner-container1">
            <div className="cube-gradient-bg cube">
              <CubeCards
                svg={svg1}
                heading={"Frontend Development"}
                para={"I build fast and responsive interfaces using React.js."}
              />
            </div>
            <div className="cube-white-bg cube shadow-bg2">
              <CubeCards
                svg={svg2}
                heading={"Backend Development"}
                para={
                  "I create secure and scalable APIs using Node.js and Express.js."
                }
              />
            </div>
          </div>
          <div className="cube-inner-container2">
            <div className="cube-white-bg cube shadow-bg1">
              <CubeCards
                svg={svg3}
                heading={"Database Management"}
                para={
                  "I manage and integrate MongoDB for dynamic data handling."
                }
              />
            </div>
            <div className="cube-gradient-bg cube">
              <CubeCards
                svg={svg4}
                heading={"Full Stack Applications"}
                para={"I build complete web apps using the MERN stack. "}
              />
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default HeroSection2;
